...
def run():
    ...
    datasets = ["camel", "cloudstack", "hadoop", "cocoon", "deeplearning", "hive", "node", "ofbiz", "qpid"]

    for dat in datasets:
        for time in directories:
            if dat == "camel" and time in ["1 day", "7 days"]: continue
            ...
            config = {
                ...
                "random": True,
                "learners": [FeedforwardDL(random={"n_layers": (2,6), "n_units": (10,20)}, verbose=0, weighted=.5, n_epochs=100)],
                "log_path": "../log/",
                "data": [data],
                "name": dat + "-" + time
            }
            for _ in range(50):
                config["learners"].append(FeedforwardDL(random={"n_layers":(2,6),"n_units":(10,20)}, verbose=0, weighted=.5, n_epochs=100))
